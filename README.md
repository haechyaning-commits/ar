...
- PDF 파일을 넣으면 탐지 → 검토(문서유형 선택, 텍스트 기반 수동 추가 포함) → 마스킹 → 자체검증 → 파일명 규칙에 맞춰 저장 → 원본 보관 이동 → 로그/요약리포트 기록(자동/수동 탐지출처 구분 포함)까지 전체 파이프라인이 실제로 동작함 (더미 데이터로 검증)
- 개발 중 발견한 버그 다수를 실제 재현 테스트로 확인 후 수정함 (예: 마스킹 텍스트 폰트 크기 축소, 전화번호/계좌번호 이중탐지, 이메일 1글자 로컬파트 미마스킹, 파일명이 겹치는 원본이 보관 폴더에서 서로 덮어쓰던 문제 등)
...
- [`mvp/review_ui.py`](mvp/review_ui.py) — 최소 검토 화면 (PySide6). 기본 전체 승인 상태에서 예외만 체크 해제, 문서유형 선택(결과 파일명에 사용), 텍스트 검색 기반 수동 추가(드래그/클릭 방식은 실제 화면 테스트 가능해지면 추가 예정)
