...
3. 검토 UI (PySide6) — v13에서 6.3.1~6.3.6이 추가되며 항목이 늘어난 만큼, 아래 하위 순서로 진행:
   ...
   5. **동일 값 일괄 처리(6.3.3)** — 2번(수동 추가)의 텍스트 다중매칭 로직을 재사용하므로 그 뒤에 진행
...
| 사용성(속도) | ⚠️ 미검증 | v12 기본 UI(전체승인+예외해제) + 6.3.5(저신뢰 항목 우선 정렬)는 구현됨(`mvp/review_ui.py`), v13 신규 6개 기능 중 나머지 5개(6.3.1~6.3.4, 6.3.6)는 미구현 — 체감 속도는 여전히 확인 불가 |
