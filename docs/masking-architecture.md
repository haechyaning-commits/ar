...
| 사용성(속도) | ⚠️ 미검증 | v12 기본 UI(전체승인+예외해제) + 6.3.3(동일 값 일괄 처리) + 6.3.4(페이지 단위 진행 상태 표시) + 6.3.5(저신뢰 항목 우선 정렬)는 구현됨(`mvp/review_ui.py`), v13 신규 6개 기능 중 나머지 3개(6.3.1/6.3.2/6.3.6)는 미구현 — 체감 속도는 여전히 확인 불가 |
