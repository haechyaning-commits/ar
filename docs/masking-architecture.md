# 감사파일 개인정보 마스킹 자동화 — 아키텍처 설계서 (v26)
...
| 마스킹 형태 | 항목별 차등 (이름/연락처/계좌/카드는 부분, 주민번호/여권번호는 완전). **계좌/카드번호는 v26에서 부분마스킹으로 확정** — 계좌는 끝 4자리만, 카드는 앞4+뒤4 노출(6.5 표 참고, 대중적으로 통용되는 표기 관행 기준) |
...
| 주민등록번호/여권번호 완전마스킹 시 자릿수를 전부 가릴지, 일부(끝자리 등) 남길지 | 전부 가림(`**-***-******`) | 식별번호는 계좌번호와 달리 "조회 가용성"보다 "복원 불가능성"이 요구사항의 핵심이라 부분마스킹 전환의 근거가 약함 — 계좌/카드번호(아래 표)와 달리 여전히 미확인 상태로 남겨둠 |
...
→ 다음에 과장님 작업을 관찰할 기회가 있으면, 남은 항목(업무상 성명 마스킹 범위, 완전마스킹 부분 전환 여부, 성씨 사전 출처, 수동 추가 기본 유형, 다중매칭 기본 동작)을 하나씩 확인해서 채워 넣는 걸 권장.
