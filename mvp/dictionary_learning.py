...
def record_candidate(word: str, direction: str, data_dir: Path | None = None) -> bool:
    """검토자 행동 1건을 후보로 기록. 임계값(3회) 도달 시 실제 사전에 조용히
    반영하고 True(=방금 반영됨)를 반환, 아니면 False.

    확인을 구하지 않음 -- 호출하는 쪽은 반환값이 True일 때 통보 문구만 보여주면 됨.
    data_dir 생략 시 모듈의 현재 DEFAULT_DATA_DIR을 매 호출마다 다시 읽음(기본
    인자로 임포트 시점에 값을 박아두면, 테스트가 격리를 위해 모듈 속성을
    monkeypatch해도 이미 정의된 함수의 기본값에는 반영되지 않는 문제가 있었음).
    """
    if data_dir is None:
...
def get_recent_promotions(
    data_dir: Path | None = None, within_days: int = RECENT_DAYS, limit: int = RECENT_LIMIT,
) -> list[Promotion]:
...
def revert_promotion(word: str, direction: str, data_dir: Path | None = None) -> None:
