...
- PDF 파일을 넣으면 탐지 → 검토(문서유형 선택 포함) → 마스킹 → 자체검증 → 파일명 규칙에 맞춰 저장 → 원본 보관 이동 → 로그/요약리포트 기록까지 전체 파이프라인이 실제로 동작함 (더미 데이터로 검증)
...
- [`mvp/review_ui.py`](mvp/review_ui.py) — 최소 검토 화면 (PySide6). 기본 전체 승인 상태에서 예외만 체크 해제, 문서유형 선택(결과 파일명에 사용)
